...
<template>
	...
	>
		...
		>
			...
			<!-- Navbar -->
			<template
				v-if="mainLayoutType === 'horizontal'"
			>
				<the-navbar-panel
							...
							<vs-collapse>
								<vs-collapse-item>
									...
									<div class="ml-10">
										<vs-list>
											<router-link
												v-for="item in $route.meta.submenu"
												:key="item.name"
												:to="item.url"
												class="menu-statico-2-link"
												:class="{
													'menu-ativo': activeLink(item.url),
												}"
											>
												<vs-list-item
													icon-pack="material-icons"
						...
						<transition :name="routerTransition">
							...
							>
								...
								>
									...
									<h6 class="mb-1 mt-4">{{ $route.meta.pageDesc }}</h6>
								</div>
							</div>
						</transition>
...
import TheNavbarPanel from "@/layouts/components/navbar/TheNavbarPanel.vue";
