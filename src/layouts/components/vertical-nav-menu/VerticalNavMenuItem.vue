...
<template>
  <div
    ...
    <div v-if="grande">
			<router-link tabindex="-1" exact :class="[{'router-link-active': activeLink}]" :to="{ name: to }" :target="target">
				<vs-icon :icon-pack="'material-icons'"  :icon="icon"/>
		...
		<div v-else>
			<div class="flex justify-center items-center">
				<vs-icon :icon-pack="'material-icons'" :icon="icon" style="margin: 2.5rem 0 0 0" :class="[activeLink ? 'show' : 'hide']" class="icone-destaque"/>
			</div>
			<!--<feather-icon v-else :class="{'w-3 h-3': iconSmall}" :icon="icon"/>-->
		</div>
  </div>
...
export default {
  ...
  computed: {
    ...
    activeLink() {
			return (this.to == this.$route.name) || (this.$route.meta.pai == this.to) ? true : false;
		},
  }
};
