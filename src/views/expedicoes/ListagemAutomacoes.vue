      ...
      <transition name="fade" v-if="selecteds.length > 0">
        <footer-doug>
          <div class="vx-col sm:w-11/12 mb-2">
            ...
            <vs-button @click="comprarEtiquetas()" class="" color="primary" type="filled" :disabled="!podeComprar.success">
              Comprar etiquetas
            </vs-button>
            </vx-tooltip>
            <vs-button class="mr-3 float-right" color="primary" type="filled">
              Calcular frete
            </vs-button>
  ...
  methods: {
    ...
    async getLimites() {
      this.$store.dispatch('automacao/verificaLimite', {params: this.dados, config: {headers: {Authorization: `Bearer ${this.melhorenvio.token}`}}}).then(response => {
        ...
        this.limites.errorMessage = 'Não foi posssível realizar conexão com a melhor envio';
      }).finally(() => {
