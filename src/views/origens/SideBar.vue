...
<template>
	...
	>
		...
		<div class="flex flex-wrap items-center p-6" slot="footer">
			...
			<vs-button type="border" color="danger" @click="$emit('closeSidebar')"
				>Cancelar</vs-button
			>
		</div>
	</vs-sidebar>
			...
			this.$validator.validateAll().then((result) => {
				if (result) {
					...
					if (this.origem.id !== null && this.origem.id >= 0) {
						obj._method = "PUT";
						this.$store
							.dispatch("updateItem", { rota: "origems", item: obj })
							.then(() => {
								this.$vs.notify({
									title: "Sucesso",
						...
						delete obj.id;
						this.$store
							.dispatch("addItem", { rota: "origems", item: obj })
							.then(() => {
								this.$vs.notify({
									title: "Sucesso",
