						...
						<div class="vx-row flex items-center">
							<div class="vx-col w-full lg:w-1/2">
								...
								<p class="font-bold text-primary text-xl">
									{{ data.lead.ddd + data.lead.telefone | VMask('(##) #####-####') }}</p>
								<p class="font-bold text-gray text-md">{{ data.lead.email }}</p>
								<p class="font-bold text-gray text-md">CPF: {{
										data.lead.cpf || '' | VMask('###.###.###-##')
									}}</p>
							</div>
							...
							<div class="vx-col w-full">
								...
								<div class="vx-col  ">
									<div class="w-full">
										...
										>
											<!--<img :src="get_img_cdn(cliente.company_plano.plano.imagem)" class="img-conquista my-4" alt="" width="150">-->
											...
											>
												...
												<li class="flex">
													...
													<div class="w-1/2 text-right text-lg">
														<vs-chip v-for="(status, index) in hotmartStatus" :key="index" v-show="index === data.status" :color="status[1]" class="product-order-status">
															{{ status[0] }}
														</vs-chip>
													</div>
												</li>
												...
												<li class="flex">
													<div class="w-1/2 text-lg">Forma de pagamento</div>
													<div class="w-1/2 text-left text-lg">
														<vs-chip>
															<i :class="formaDepagamento.icon" class="mr-3"></i>
										...
										>
											...
											>
												...
												<li class="flex">
													...
													<div class="w-1/2  text-left text-lg">{{ data.currency }}
														{{ formatPrice(data.cms_vendor) }}
													</div>
												...
												<li class="flex">
													...
													<div class="w-1/2  text-left text-lg">{{ data.currency }}
														{{ formatPrice(data.cms_marketplace) }}
													</div>
	...
	methods: {
		pesquisarTrans(e) {
			...
			if (e)
				e.preventDefault();
			this.$vs.loading();
			this.$store.dispatch('comissoes/searchTrans', {
				produto_id: this.data.lead_produto.produto_id,
				comissao: true, ...this.dados
			}).then(response => {
		...
		storeTransacao() {
			this.$validator.validateAll().then(result => {
				if (result) {
					this.$vs.loading();
					this.$store.dispatch('comissoes/storeTrans', {
						produto_id: this.data.lead_produto.produto_id,
						email: this.transacao.email,
						payment_type: this.transacao.payment_type.id
					}).then(() => {
						this.$vs.notify({
							color: 'success',
							title: '',
