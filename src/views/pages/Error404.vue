...
<template>
	<div class="h-screen flex w-full bg-img">
		...
		>
			...
			<p class="sm:mx-0 mx-4 mb-4 sm:mb-16 d-theme-text-inverse text-dark">
				Aparentemente você estão tentando acessar uma página que não existe
				ou você não possui acesso.
			</p>
			<vx-tooltip position="top" :text="text" class="text-center">
				
				<vs-button size="large" @click.prevent="handleBack('/')"
					>Go Back</vs-button
				>
			</vx-tooltip>
		</div>
...
export default {
	...
	created() {
		...
		// if (local != null) {
		// 	//Verficando existência dos parâmetros auxiliares de redirecionamento no localStorage
		// 	this.text = local.text || "";
		// 	this.back = local.back || "";
		// } else if (this.$route.params.back || this.$route.params.text) {
		// 	//Recuperando parâmetros auxíliares pelos parâmetros da rota caso exista
		...
		// 		"404Params",
		// 		JSON.stringify({ back: this.back, text: this.text })
		// 	);
		// }
	},
	methods: {
		redirect() {
			//Limpa o localStorage para que não haja conflito de parâmetros e redireciona o usuário
