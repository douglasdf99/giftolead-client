                ...
                <div class="com-item" v-else>
                    <vs-table v-model="selected" :data="historico" @selected="handleSelected" class="table-items">
                      ...
                      <template slot="thead">
                        ...
                        <vs-th>Contato</vs-th>
                        <vs-th>Tipo</vs-th>
                        <vs-th>Data de envio</vs-th>
                        <vs-th>Tentativas</vs-th>
                        <vs-th>Criado data</vs-th>
                      </template>

                      <template slot-scope="{data}">
                        <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data" class="mb-3 cursor-pointer">
                          <vs-td>{{ tr.contato ? tr.contato.email : '' }}</vs-td>
                          <vs-td>{{ tipo(tr.jobs.payload) }}</vs-td>
                          <vs-td>{{( tr.jobs.available_at * 1000)| formatDateTimeMili }}</vs-td>
                          <vs-td>{{ tr.jobs.attempts }}</vs-td>
