<template>
	<div>
					...
					<div class="vx-col w-full lg:w-10/12">
						<div class="vx-row">
							<div class="vx-col w-1/2">
								...
								<span class="text-danger text-sm" v-show="errors.has('periodo')">{{
		...
		<div class="vx-row">
			<div class="vx-col w-full xlg:w-8/12 lg:w-9/12">
				<div class="vx-row mb-3">
					<div class="vx-col w-full">
						<span class="font-regular mb-3">Mensagem</span>
						<Editor v-if="email.corpo" :content="email.corpo" @ready="onEditorReady($event)" @changed="onChange"/>

					</div>
				</div>
			</div>
			<div class="vx-col w-full xlg:w-4/12 lg:w-3/12">
			...
			<footer-doug>
				<div class="vx-col sm:w-11/12 mb-2">
					...
					<vs-button class="float-right mr-3" color="dark" type="border" icon-pack="feather" icon="x-circle"
								  @click="$router.push({
									name: 'campanha-config-checkout-emails', params:{id: $route.params.id}
									})">
						Cancelar
					</vs-button>
...
export default {
	name: "EmailsCriar",
	components: {
		Editor, 'v-select': vSelect, quillEditor, PhoneNumber
	},
	async created() {
		...
		if (this.$route.name === 'campanha-config-checkout-emails-editar') {
			this.email.corpo = '';
			await this.getId(this.$route.params.idEmail);
		} else
			await this.confereCampanha();
		...
		salvar() {
			...
			if (this.email.id !== undefined) {
				...
				this.$store.dispatch('checkout/updateEmail', {
					...
					this.$router.push({
						name: 'campanha-config-checkout-emails', params:{id: this.$route.params.id}
						});
				}).catch(erro => {
					this.$vs.notify({
						title: 'Error',
				...
				this.$store.dispatch('checkout/storeEmail', this.email).then(() => {
					this.$vs.notify({
						title: '',
					...
					this.$router.push({name: 'campanha-config-checkout-emails', params:{id: this.$route.params.id}});
				}).catch(erro => {
					this.$vs.notify({
						title: 'Erro',
						text: erro.response.data.message,
