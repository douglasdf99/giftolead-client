	...
	<div>
		<div class="vx-row mb-4">
            ...
            <span class="float-right mt-1 mx-4" style="font-weight: bold">
				{{ campanha.status ? 'Ativado' : 'Desativado' }}
				</span>
			...
			<div class="vx-col w-full lg:w-8/12">
				<div class="vx-row">
					<div class="vx-col w-full mb-4">
						...
						<vs-input class="w-full" id="search_input_trans" v-model="campanha.checkout" placeholder="https://"
									 size="large" name="nome" v-validate="'required'"/>
					</div>
					<div class="vx-col w-full">
						<div class="my-8">
							<vs-checkbox color="dark" v-model="campanha.webhook_handle" @click="notifyWebhook(campanha.webhook_handle)">
								<span class="label-bold-underline">Utilizar WebHook de carrinhos abandonados</span>
							</vs-checkbox>
							<small class="flex mt-2 ml-3"><i class="material-icons text-base mr-2">info_outline</i>
								Essa opção possibilita receber os dados de abandono de carrinho vindos da hotmart</small>
						</div>
					</div>
				</div>
		...
		<custom-popup :active.sync="modal" title="Tipo de Envio">
			<div class="vx-row">
				<div class="vx-col w-full tipo-conquista mx-10 my-5 flex items-center hover-opacidade cursor-pointer"
					  @click="configEnvio('emails')">
...
export default {
	...
	methods: {
		...
		configEnvio(val) {
			...
			if (val == 'emails') {
				if (this.$acl.check('planos_campanhas_config_email'))
					this.$router.push({
						name: `campanha-config-checkout-${val}`, params:{id: this.$route.params.id}
						});
				else {
					this.$vs.notify({
						color: 'danger',
		...
		agendados() {
			this.$router.push({
				name: 'campanha-config-checkout-agendados', params: {id: this.$route.params.id}
				});
		},
		historico() {
			this.$router.push({
				name: 'campanha-config-checkout-historico', params:{id: this.$route.params.id}
				});
		},
		contatos(val) {
			this.$router.push({
				name: `campanha-config-checkout-contatos-${val}`, params:{id: this.$route.params.id}
				});
		},
		notifyWebhook() {
			if(!this.campanha.webhook_handle) {
				Vue.swal({
					title: "Atenção",
					text: "Esta opção só pode ser habilitada em uma única Campanha de Carrinho. Ao ativa-la, ela desabilita a mesma nas outras campanhas.",
					type: "warning",
					...
					if(!result.isConfirmed)
						this.campanha.webhook_handle = !this.campanha.webhook_handle;
				});
			}
		},
	},
