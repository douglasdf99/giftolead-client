   ...
   <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary"
      ...
      <div class="flex flex-wrap items-center p-6" slot="footer">
         <vs-button class="mr-6" @click="submitData"
                    :disabled="verificaLeadEmail || !selectedOrigem.id || !selectedDuvida.id">{{
...
import { phoneNumberInputTranslations } from '../../../saveleadsConfig';
...
export default {
   ...
   methods: {
      ...
      initValues() {
         if (this.data.id) {
            ...
            this.selectedDuvida = this.empty;
            this.selectedProduto = this.empty;
            this.selectedOrigem = this.empty;
         }
      },
      submitData() {
   ...
   async created() {
      ...
      if (Object.entries(this.data).length === 0) {
         ...
         if (this.ticket.produto.id)
            this.selectedProduto = {id: this.ticket.produto.id, label: this.ticket.produto.nome};

         if (this.ticket.tipoduvida.id)
            this.selectedDuvida = {id: this.ticket.tipoduvida.id, label: this.ticket.tipoduvida.nome};

         if (this.ticket.origem.id)
            this.selectedOrigem = {id: this.ticket.origem.id, label: this.ticket.origem.nome};
      }
