					...
					<div class="w-full p-5 rounded-lg" style="background-color: #EDEDED">
						<div class="vx-row mb-4">
							<div class="vx-col lg:w-4/12 md:w-1/3 sm:w-full">
								<div class="card-finalizacao ganhou bg-white" v-bind:class="{'tipoAtivo' : (atendimento.tipo == 0)}" @click="atendimento.tipo = 0; selectedStatus = ''; datetime = null">
									<span>Ganhou</span>
									<img src="@/assets/images/util/ganhou.svg" />
								</div>
							</div>
							<div class="vx-col lg:w-4/12 md:w-1/3 sm:w-full">
								<div class="card-finalizacao aguardando bg-white" v-bind:class="{'tipoAtivo' : (atendimento.tipo == 1)}" @click="atendimento.tipo = 1; selectedStatus = ''; habbrinde = false; habperda = false;">
							...
							<div class="vx-col lg:w-4/12 md:w-1/3 sm:w-full">
								<div
									class="card-finalizacao perdeu bg-white"
							...
							<div class="vx-col w-full lg:w-11/12 ml-auto">
								<ul class="list-tipo-comissao mt-2">
									<span class="text-danger text-sm"
											v-show="errors.has('foobar')">Preenchimento obrigatório</span>
									<li class="my-3" v-for="item in statusGanhou" @click="verificaHabBrinde(item)" :key="item.nome">
						...
						<div class="vx-row" v-if="atendimento.tipo == 1">
							...
							<div class="vx-col w-full lg:w-3/12 ml-10 relative">
								<span class="text-danger text-sm"
										v-show="errors.has('datetime')">Preenchimento obrigatório</span>
								<flat-pickr
									:config="configdateTimePicker"
									v-model="datetime" name="datetime"
									v-validate="'required'" id="teste"
							...
							<div class="vx-col w-full lg:w-11/12 ml-auto">
								<ul class="list-tipo-comissao mt-2">
									...
									<li class="my-3" v-for="(item, index) in statusAguardando" :key="index">
										<label data-v-7d8956fe="" class="vs-component con-vs-radio hover:font-bold vs-radio-dark">
											<input name="foobar" v-validate="'required'" v-model="selectedStatus" type="radio" class="vs-radio--input" :value="item.id">
											<span class="vs-radio">
						...
						<div class="vx-row" v-if="atendimento.tipo == 2">
							<div class="vx-col w-full lg:w-4/12">
								...
								<ul class="list-tipo-comissao mt-2 ml-10">
									<span class="text-danger text-sm"
											v-show="errors.has('foobar')">Preenchimento obrigatório</span>
									<li class="my-3" v-for="item in statusPerdeu" @click="verificaHabPerda(item)" :key="item.id">
										<label data-v-7d8956fe="" class="vs-component con-vs-radio hover:font-bold vs-radio-dark">
											<input name="foobar" v-validate="'required'" v-model="selectedStatus" type="radio" class="vs-radio--input" :value="item.id">
											<span class="vs-radio">
												<span class="vs-radio--borde"
														style="border: 2px solid rgb(200, 200, 200);"></span>
												<span class="vs-radio--circle"></span>
											</span>
											<span class="vs-radio--label">{{ item.nome }}</span>
										</label>
					...
					<transition name="fade">
						<div class="vx-row mb-10" v-if="atendimento.tembrinde">
							...
							<div class="vx-col w-full lg:w-1/3">
								...
								<v-select v-model="selectedBrinde" :class="'select-large-base'" :clearable="false" style="background-color: white" :options="brindesOptions" v-validate="'required'" name="brinde"/>
					...
					<transition name="fade">
						<div class="vx-row mb-10" v-if="atendimento.hasupsell">
							...
							<div class="vx-col w-full lg:w-1/3 mb-10">
								...
								<v-select v-model="selectedUpsell" :class="'select-large-base'" :clearable="false" style="background-color: white" :options="opcoes" v-validate="'required'" name="produtoUpsell"/>
			...
			<transition name="fade">
				<footer-doug>
					<div class="vx-col sm:w-11/12 mb-2">
						<vs-button class="float-right mr-3" color="dark" type="border" icon-pack="feather" icon="x-circle" @click="cancelarAtendimento(ticket.id)">
							Cancelar Atendimento
						</vs-button>
						<button type="submit" name="button" :disabled="isInvalid" class="float-right vs-component vs-button mr-3 vs-button-primary vs-button-filled">
...
export default {
	...
	methods: {
		...
		finalizar() {
			...
			this.atendimento.produto_id = this.ticket.produto_id;
			this.atendimento.tipo_de_perda_id = this.selectedMotivo.id;
			this.$validator.validateAll().then(result => {
				if (result) {
					this.$vs.dialog({
						...
						accept: () => {
							this.$vs.loading();
							this.$store.dispatch('tickets/finalizar', this.atendimento).then(() => {
								this.$vs.notify({
									color: 'success',
									title: '',
