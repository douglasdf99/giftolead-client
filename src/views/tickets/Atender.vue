<template>
	<div>
		...
		<links v-if="listLinks" :isSidebarActive="listLinks" @getId="getId($route.params.id)" @closeSidebar="toggleLinksSidebar" :data="aresponder"/>
						...
						<vx-card class="p-2">
							...
							<div class="flex items-center">
								<div class="fill-row-loading w-full">
									...
									<div id="loading-sound" :class="{'activeLoading':activeLoading}" class="vs-con-loading__container loading-example w-full" v-show="chamada.id" />
									<div id="loading-default" :class="{'activeLoading':activeLoading}" class="vs-con-loading__container loading-example w-full" v-show="!chamada.id" />
									<h5><span class="font-14 my-2">Tempo:</span> {{ time }}</h5>
								</div>
							</div>
							<div class="flex justify-center flex-wrap mt-10">
								<vs-button size="large" class="font-bold mx-2 rounded-full" color="success" type="filled" v-if="!muted" icon-pack="material-icons" icon="volume_up" @click="mute">
								</vs-button>
								<vs-button size="large" class="font-bold mx-2 rounded-full" color="danger" type="filled" v-else icon-pack="material-icons" icon="volume_off" @click="mute">
								</vs-button>
								<vs-button size="large" class="font-bold mx-2 rounded-full" color="danger" type="filled" icon-pack="material-icons" icon="call_end" @click="desligaChamada">
								</vs-button>
							</div>
							<template slot="footer">
								<vs-divider/>
								<div class="flex justify-between">
									<span class="flex items-center"><vs-icon icon="signal_cellular_alt"/><span>Internet: {{ status.internet }} </span></span>
									<span class="flex items-center"><vs-icon icon="computer"/><span>computador: {{ status.computador }}</span></span>
								</div>
							</template>
						</vx-card>
					</div>
					...
					<div class="vx-row my-4">
						...
						<div class="vx-col w-3/4" v-if="ticket.lead">
							...
							<p class="text-black text-xl font-bold mb-2" v-if="ticket.lead.telefone && !editable">
								...
								<span class="mt-5"><vs-icon icon="create" icon-pack="material-icons" style="color: #9344C4;font-size: 1.2rem;cursor: pointer;margin-top: 5px" @click="editable=true; ticket.lead.telefone=ticket.lead.ddd+ticket.lead.telefone"/></span>
							</p>
							<div class="vx-row my-2">
								<div class="vx-col w-9/12" v-if="editable">
									<VuePhoneNumberInput
										...
										no-flags required no-use-browser-locale
							...
							<div class="w-full flex my-5">
								...
								<vx-tooltip text="Enviar whatsapp" position="top">
									<vs-button class="rounded-full mx-2 px-1 py-2" color="#8ED839" type="filled" @click="whatsapp(ticket)">
								...
								<vx-tooltip text="Links de venda" position="top">
									<vs-button class="rounded-full mx-2 px-1 py-2" color="#181830" type="filled" @click="links(ticket)">
					...
					<div class="vx-row my-4 flex justify-between">
						...
						<div class="vx-col w-full lg:w-1/2">
							...
							<p class="text-right flex items-center font-bold float-right" v-if="ticket.responsavel">
								...
								<vs-avatar v-else color="primary" size="40px" :text="ticket.responsavel.name" />

								<img :src="null" width="40" class="ml-2 rounded-full" v-if="ticket.responsavel.avatar" :style="{ backgroundImage: 'url('+get_img_cdn(ticket.responsavel.avatar)+')',backgroundRepeat: 'no-repeat',backgroundSize:'cover', width: '60px', height:'60px' }">
							</p>
						</div>
					</div>
							...
							<template slot-scope="{data}">
								<vs-tr :key="indextr" v-for="(tr, indextr) in data" :data="tr">
									...
									<vs-td :data="data[indextr].status" class="td-icons flex flex-col items-center justify-center">
										<vs-icon icon-pack="material-icons" icon="fiber_manual_record" class="icon-grande" v-bind:style="{color: getStatusSoli(tr.status)}" v-if="data[indextr].status"></vs-icon>
									</vs-td>
								</vs-tr>
							</template>
						</vs-table>
...
export default {
	...
	methods: {
		...
		getId(id) {
			this.$vs.loading();
			this.$store.dispatch('tickets/getId', id).then(() => {
				this.findddi();
			}).finally(() => this.$vs.loading.close());
		},
		...
		finalizar() {
			this.$vs.dialog({
					...
					accept: () => {
						this.$vs.loading();
						this.$store.dispatch('tickets/finalizar', this.ticket).then(() => {
							this.$vs.notify({
								color: 'success',
								title: '',
		...
		verificacao() {
			this.$store.dispatch('tickets/verificaDisponibilidade', this.$route.params.id).then(response => {
				...
				if (erro.response.status == 404) this.$router.push({
					name: 'page-error-404',
					params: {back: 'tickets-list', text: 'Retornar à listagem de Tickets'}
				});
			}).finally(() => this.$vs.loading.close());
			localStorage.removeItem('atendimento');
		},
		...
		,
		sendChamada(id) {
			this.$store.dispatch('tickets/chamaNumero', {ticket: this.ticket, chamada_id: id}).then(() => {
			});
		}
		,
		getStatus(val) {
