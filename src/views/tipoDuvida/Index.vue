				...
				<div class="flex items-center">
					<div class="relative w-full">
						...
						<form @submit="pesquisar">
							<vs-input autocomplete class="w-full vs-input-shadow-drop vs-input-no-border d-theme-input-dark-bg" v-model="dados.search" id="search_input" size="large" placeholder="Pesquisar por nome"/>
							<!-- SEARCH LOADING -->
							<!-- SEARCH ICON -->
						...
						<template slot-scope="{data}">
							<vs-tr :key="indextr" v-for="(tr, indextr) in data" class="mb-3 relative">
								<vs-td class="flex justify-center items-center relative w-full">
									<vs-dropdown vs-trigger-click v-show="tr.company_id" v-if="($acl.check('configuracao_tipoduvida_editar') || $acl.check('configuracao_tipoduvida_deletar') && tr.company_id)">
										<vs-button radius color="#EDEDED" type="filled" class="btn-more-icon relative botao-menu" icon-pack="material-icons" icon="more_horiz" />
...
export default {
	...
	methods: {
		...
		getItems() {
			this.$store.dispatch('getVarios', {rota: 'tipo_de_duvidas', params: this.dados}).then(response => {
		...
		deletar(id) {
			this.$vs.dialog({
				color: 'danger',
				title: `Deletar tipo de dúvida id: ${id}`,
				text: 'Deseja deletar este tipo de dúvida? Procedimento irreversível',
				acceptText: 'Sim, deletar!',
				accept: () => {
					this.$vs.loading();
					this.$store.dispatch('deleteItem', {id: id, rota: 'tipo_de_duvidas'}).then(() => {
						this.$vs.notify({
							color: 'success',
							title: 'Sucesso',
							text: 'O tipo de dúvida foi deletado com sucesso'
						});
						this.getItems();
					}).catch(() => {
						this.$vs.notify({
