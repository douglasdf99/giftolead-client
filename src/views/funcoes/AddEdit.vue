	...
	<div>
		<div class="vx-row mb-10">
			...
			<div
				...
				v-if="$acl.check('configuracao_funcao_fullprod')"
			>
				<vs-checkbox v-model="funcao.full_products"
					>Permitir todos os produtos</vs-checkbox
				>
			</div>
			<div class="vx-col w-full" v-if="!funcao.full_products">
				...
				<v-select
					multiple
					:closeOnSelect="false"
					v-model="produtos_permitidos"
					:options="all_produtos"
					class="bg-white"
				/>
			</div>
		</div>
		<div class="vx-row mt-10" v-if="modules.length > 0">
					...
					>
						<div slot="header">
							<vs-checkbox
								...
								v-model="main_arr_permissions"
								@click="checkModules(main)"
								:vs-value="main.id"
						...
						<div class="vx-row my-4">
							<div
								class="vx-col w-full lg:w-1/4 mb-3"
								v-for="(sub_module, index) in getDependentes(
									main.module
								)"
								:key="index"
							>
								...
								<div class="flex items-center my-3">
									<vs-checkbox
										v-model="main_arr_permissions"
										@click="checkModules(sub_module)"
										:vs-value="sub_module.id"
										><span class="text-lg">{{
								...
								<div
									class="ml-4 my-2"
									v-show="hasDependentes(sub_module.module)"
									v-for="(ter_module, index) in getDependentes(
										sub_module.module
									)"
									:key="index"
								>
									...
									<div class="flex items-center">
										<vs-checkbox
											v-model="main_arr_permissions"
											@click="checkModules(ter_module)"
											:vs-value="ter_module.id"
											><span class="text-md">{{
												ter_module.module_name
											}}</span>
										</vs-checkbox>
									</div>
									<div
										class="ml-4 my-2"
										v-show="hasDependentes(ter_module.module)"
										v-for="(quar_module, index) in getDependentes(
											ter_module.module
										)"
										:key="index"
									>
										...
										<div class="flex items-center">
											<vs-checkbox
												v-model="main_arr_permissions"
												@click="checkModules(quar_module)"
												:vs-value="quar_module.id"
												><span class="text-md">{{
													quar_module.module_name
												}}</span>
											</vs-checkbox>
										</div>
										<div
											class="ml-4 my-2"
											v-show="hasDependentes(quar_module.module)"
											v-for="(quin_module, index) in getDependentes(
												quar_module.module
											)"
											:key="index"
										>
											...
											<div class="flex items-center">
												<vs-checkbox
													v-model="main_arr_permissions"
													@click="checkModules(quin_module)"
													:vs-value="quin_module.id"
													><span class="text-md">{{
...
export default {
	...
	methods: {
		salvar() {
			this.$validator.validateAll().then((result) => {
				if (result) {
					...
					this.funcao.full_products = this.funcao.full_products ? 1 : 0;
					this.$vs.loading();
					this.$store
						.dispatch("funcoes/store", this.funcao)
						.then((response) => {
							this.$vs.notify({
								title: "",
		...
		hasDependentes(name) {
			//Verifica se o módulo possui módulos dependentes dele
			let tem = false;
			this.permissoes.forEach((perm) => {
				if (perm.module_depends === name) tem = true;
			});
			return tem;
		},
		getDependentes(name) {
			//Busca os módulos dependentes do verificado
			let arr = [];
			this.permissoes.forEach((perm) => {
		...
		hasPai(name) {
			//Verifica se o módulo em questão é dependente de outro
			let tem = false;
			this.permissoes.forEach((perm) => {
				if (perm.module === name) tem = true;
			});
			return tem;
		},
		getPai(name) {
		...
		checkModules(modulo) {
			//Executado ao clicar no checkbox.
			if (this.clicked(modulo.id)) {
				//Caso ele esteja "preenchido"
		...
		checkPai(modulo) {
			//Função executada em loop até encontrar toda a árvore de herança ascendente a ser colocada no array de permissões
			if (this.hasPai(modulo.module_depends)) {
				let obj = this.getPai(modulo.module_depends); //Pega as informações do módulo pai
				if (this.main_arr_permissions.indexOf(obj.id) === -1) {
					//Verifica a existência dele no array
		...
		pushOption(modulo) {
			//Rotina de inserção de dependentes no array de permissões ao clicar em alguma que seja "pai" de outras
			if (this.hasDependentes(modulo.module)) {
				...
				array.forEach((item) => {
					if (this.main_arr_permissions.indexOf(item.id) === -1) {
						//Verifica se já não se encontra no array principal
		...
		removeOption(modulo) {
			//Rotina similar à anterior, porém, buscando remover os módulos dependentes.
			if (this.hasDependentes(modulo.module)) {
				let array = [];
		...
		tratarPermissions(array) {
			//Joga as permissões que a Função já possui dentro do array principal.
		...
		setProdutosPermitidos() {
			//Aloca os produtos que a função já tem acesso
	...
	computed: {
		...
		modules() {
			//Tratando módulos principais
			let arr = [];
			this.permissoes.forEach((perm) => {
				if (!perm.module_depends && !perm.depends) {
					//módulos que não possuem ascendência são considerados "principais". Ex.: Configurações, Tickets, Brindes
					arr.push(perm);
				}
			});
