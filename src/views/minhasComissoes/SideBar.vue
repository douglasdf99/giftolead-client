		...
		<VuePerfectScrollbar class="scroll-area--data-list-add-new" :key="$vs.rtl">
			<div class="p-10">
							...
							<div class="vx-row mb-4" v-if="outroemail">
								<div class="vx-col w-full mb-4">
									...
									<div class="w-full text-black text-sm p-3" style="background-color: #EDEDED">
										A comissão passará por uma análise manual, e se faz nescessário de prints comprovando a
										venda por esse email.
									</div>
								</div>
							</div>
						</transition>
								...
								<label for="file">
									<div class="vx-row">
										<div class="vx-col w-full text-center">
											...
											<img :src="get_img_cdn('images/upload.png')" v-if="images.length == 0"/>
				...
				<div v-else>
					<div class="vx-row flex items-center">
						<div class="vx-col w-full lg:w-1/2" v-if="data.ticket.lead.nome">
							...
							<p class="font-bold text-primary text-xl" v-if="data.ticket.lead.telefone">
								{{ data.ticket.lead.ddd + data.ticket.lead.telefone | VMask('(##) #####-####') }}</p>
							<p class="font-bold text-gray text-md">{{ data.ticket.lead.email }}</p>
							<p class="font-bold text-gray text-md" v-if="data.ticket.lead.cpf">CPF:
								{{ data.ticket.lead.cpf || '' | VMask('###.###.###-##') }}</p>
						</div>
						<div class="vx-col w-full lg:w-1/2 text-right">
							...
							<vs-chip class="float-right" :color="data.tipo === 'pendente' ? 'warning' : 'danger'">Status:
								{{ data.tipo }}
							</vs-chip>
						</div>
					</div>
				...
				<div class="vx-row px-3">
					...
					<galeria v-if="data.anexos" :imagens="data.anexos" @removeImg="removeImg"
								:remove="data.action == 1"></galeria>
				</div>
			</div>
		</VuePerfectScrollbar>
...
export default {
	...
	methods: {
		diags() {
			var self = this;
			for (const item of self.files.entries()) {
				self.enviar2(item[1]).then(() => {
			...
			return new Promise((resolve) => {
				...
				if (this.files.length > 0) {
					this.files.forEach(file => {
						...
						this.$store.dispatch('mcomissoes/setAnexos', formData);
					});
				} else {
					formData.append('arquivo', null);
					formData.append('pre_comissao_id', this.data.id);
