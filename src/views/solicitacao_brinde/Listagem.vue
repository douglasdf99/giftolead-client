        ...
        <transition name="fade" v-if="selecteds.length > 0">
            <footer-doug>
                <div class="vx-col sm:w-11/12 mb-2">
                    <vs-button class="float-right mr-3 text-black" color="danger" type="border" icon-pack="material-icons" icon="close"
                               @click="$emit('aprovarVarias', selecteds, 'reprovar')" v-if="tipo == 'pendente'" :disabled="!$acl.check('brinde_solicitacao_reprovar')">
                        Reprovar Etiquetas
                    </vs-button>
                    <vs-button class="float-right mr-3 text-black" color="primary" type="border" icon-pack="material-icons" icon="check" @click="$emit('aprovarVarias', selecteds, 'restaurar')"
                               v-if="tipo == 'reprovado'" :disabled="!$acl.check('brinde_solicitacao_aprovar')">
                        Restaurar Etiquetas
                    </vs-button>
                    <vs-button class="float-right mr-3" color="primary" type="filled" @click="$emit('aprovarVarias', selecteds, 'aprovar', tipo)"
