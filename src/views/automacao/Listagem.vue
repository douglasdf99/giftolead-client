            ...
            <vs-table :data="items" class="table-items">
                ...
                <template slot-scope="{data}">
                    <vs-tr v-for="tr in data">
                            ...
                            <vs-dropdown vs-trigger-click
                                ...
                                <vs-dropdown-menu class="dropdown-menu-list">
                                    <vs-dropdown-item @click="editarEndereco(tr)"
                                                      v-if="$acl.check('brinde_automacao_editar') && !tr.expedicao">
                                        <vs-icon icon-pack="material-icons" icon="create"></vs-icon>
                                        Editar endereço
                                    </vs-dropdown-item>
                                    <vs-dropdown-item @click="arquivar(tr)"
                                                      v-if="$acl.check('brinde_automacao_deletar')">
                                        <vs-icon icon-pack="material-icons" icon="work"></vs-icon>
                                        Arquivar
                                    </vs-dropdown-item>
                                    <vs-dropdown-item @click="restaurarAlert(tr.id)" v-show="tr.arquivado == 1"
                                                      v-if="$acl.check('brinde_automacao_deletar')">
                        ...
                        <vs-td class="flex justify-center">
                            <vx-tooltip text="Enviando" position="top">
                                <vs-icon icon-pack="material-icons" icon="watch_later" color="gray"
                                         v-if="tr.eventos.length == 0" class="text-2xl"/>
                            </vx-tooltip>
                            <vx-tooltip text="Não enviado" position="top">
                                <vs-icon icon-pack="material-icons" icon="cancel" color="danger" v-if="getEventoRed(tr)"
                                         class="text-2xl"/>
                            </vx-tooltip>
                            <vx-tooltip text="Enviado" position="top">
                                <vs-icon icon-pack="material-icons" icon="check" color="success"
                                         v-if="getEventoGreen(tr)" class="text-2xl"/>
                            </vx-tooltip>
                        </vs-td>
                        <vs-td>
                            {{ tr.uuid }}
                        ...
                        <vs-td class="flex justify-center items-center text-center">
                            <vx-tooltip position="top" :text="tr.rastreio">
                                <img src="@/assets/images/util/delivery-icon.svg" width="40px"
                                     class="mr-2 cursor-pointer" @click="copyText(tr.rastreio)"
                                     v-if="tr.rastreio != null">
                            </vx-tooltip>
                            <vx-tooltip position="top" :text="`Expedição nº ${tr.expedicao_id}. Clique para detalhar.`">
                                <img src="@/assets/images/util/expedicao-icon.svg" width="25px"
                                     class="rounded-full shadow-lg cursor-pointer hover:shadow-none" @click="detailExpedicao(tr.expedicao)"
