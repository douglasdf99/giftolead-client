...
<template>
   <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary"
      ...
      <VuePerfectScrollbar class="scroll-area--data-list-add-new" :key="$vs.rtl">
         <div class="p-6">
            <vs-alert color="danger" v-if="!brinde.contrato.status && this.data.length > 0" icon-pack="material-icons"
                      icon="cancel" class="w-full">
            ...
            <div class="p-5" v-if="Object.entries(this.data).length != 0">
               ...
               <vs-switch vs-icon-on="check" color="#0FB599" v-model="brinde.ativo" class="float-right switch"
                          @click="switchBrinde(brinde.ativo)"/>
            </div>
            <vs-input size="large " v-validate="'required'" label="Nome do brinde" autocomplete="off"
                      v-model="brinde.nome"
                      class="mt-5 w-full"
                      name="nome"/>
            ...
            <div class="vx-row mt-base">
               ...
               <div class="vx-col w-full lg:w-1/2 mb-base">
                  <vx-tooltip position="top"
                              :text="checkExtension('correios') ? 'Contrato dos Correios' : 'Nenhum Contrato de Correios ativo'">
                     <vx-card class="items-center cursor-pointer card-option"
                              :class="{'border-primary' : brinde.contrato_type === 'App\\Models\\Correio', 'disabled': !checkExtension('correios')}"
                              @click="brinde.contrato_type = 'App\\Models\\Correio'">
                        <div slot="no-body">
                           <div class="p-4">
                              <img src="@/assets/images/util/correios.png"
                                   class="mx-auto rounded-full lg:w-1/3 mx-auto">
                           </div>
               ...
               <div class="vx-col w-full lg:w-1/2 mb-base">
                  <vx-tooltip position="top"
                              :text="checkExtension('melhor_envio')? 'Extensão Melhor Envio' : 'Extensão não instalada'">
                     <vx-card class="items-center card-option cursor-pointer"
                              @click="brinde.contrato_type = 'App\\Models\\Extensoes\\MelhorEnvio'"
                              :class="{'border-primary' : brinde.contrato_type === 'App\\Models\\Extensoes\\MelhorEnvio', 'disabled': !checkExtension('melhor_envio')}">
                        <div slot="no-body">
                           <div class="p-4">
                              <img src="@/assets/images/util/melhor-envio.png"
                                   class="mx-auto rounded-full w-full lg:w-1/3 mx-auto">
                           </div>
                           <div class="mx-auto flex">
               ...
               <div class="vx-col w-full" v-if="brinde.contrato_type == 'App\\Models\\Correio'">
                  ...
                  <v-select v-model="selected" :class="'select-large-base'" :clearable="false" :options="opcoesContrato"
                            v-validate="brinde.contrato_type == 'App\\Models\\Correio' ? 'required' : ''"
                            name="contrato"
                            v-bind:style="{border: (!brinde.contrato.status && this.data.length > 0 ? '2px solid #ff000066' : '')}"
                            :disabled="opcoesContrato.length === 0"/>
                  <span class="text-danger text-sm" v-show="errors.has('contrato')">{{
                        errors.first('contrato')
                     }}</span>
                  <span class="text-danger text-sm" v-if="opcoesContrato.length === 0">Você não possui nenhum contrato com os correios ativo</span>
               </div>
            </div>
            <vs-divider></vs-divider>
            <div class="p-4 pt-0 card-contexto">
               <div class="p-4 mt-4 border-rounded bg-card-gray">
                  <h6 class="font-bold  text-center ">Tipo de Embalagem</h6>
               </div>
               <div class="mt-8">
                  <vs-checkbox color="dark" v-model="brinde.hasembalagem"><span class="label-bold-underline">Usar embalagem padrão</span>
                  </vs-checkbox>
               </div>
               <div class="" v-show="!brinde.hasembalagem">
                  ...
                  <div class="vx-row mt-5">
                     <div class="vx-col  w-full mb-2">
                        ...
                        <v-select v-model="brinde.tipo_de_caixa" :class="'select-large-base'" :clearable="false"
                                  :options="opcoesTipoCaixa" v-validate="!brinde.hasembalagem ? 'required' : ''" name="tipo_de_caixa"/>
                        <span class="text-danger text-sm"
                              v-show="errors.has('tipo_de_caixa')">{{ errors.first('tipo_de_caixa') }}</span>
                     </div>
                     <div class="vx-col sm:w-1/2 w-full mb-2 relative" v-if="tipo_caixa">
                        <vs-input size="large" v-validate="tipo_caixa ? 'required' : ''" label="Altura" autocomplete="off"
                     ...
                     <div class="vx-col sm:w-1/2 w-full mb-2 relative" v-if="tipo_caixa">
                        <vs-input size="large " v-validate="tipo_caixa ? 'required' : ''" label="Largura" autocomplete="off"
                                  v-model="brinde.largura" class="mt-5 w-full" type="number" name="largura"/>
                        <div class="unidade absolute p-2">
                           <span>cm</span>
                        </div>
                        <span class="text-danger text-sm" v-show="errors.has('largura')">{{
                  ...
                  <div class="vx-row">
                     <div class="vx-col sm:w-1/2 w-full mb-2 relative" v-if="tipo_cilindro">
                        <vs-input size="large " v-validate="tipo_cilindro ? 'required' : ''" label="Diâmetro" autocomplete="off"
                     ...
                     <div class="vx-col sm:w-1/2 w-full mb-2 relative" v-if="tipo_caixa || tipo_cilindro">
                        <vs-input size="large " v-validate="(tipo_caixa || tipo_cilindro) ? 'required' : ''" label="Comprimento" autocomplete="off"
   ...
   data() {
      return {
         brinde: {
            ...
            tipo: true,
            contrato: {},
            tipo_de_caixa: {id: null, label: 'Selecione o tipo da embalagem'},
            contrato_type: 'App\\Models\\Correio'
         },
         tipo_caixa: false,
         tipo_envelope: false,
         tipo_cilindro: false,
   ...
   methods: {
      ...
      submitData() {
         this.$validator.validateAll().then(result => {
            if (result) {
               ...
               if (this.brinde.id !== null && this.brinde.id >= 0) {
                  ...
                  delete obj.id;
                  obj.integracao_id = this.selected ? this.selected.id : null;
		...
		checkExtension(value) {
			return this.$store.getters['extensoes/check'](value);
		},
      getEmbalagems() {
