			...
			<div class="vx-col w-full lg:w-1/3 sm:w-1/2 mb-base">
				<vx-card title="Atividades" no-shadow>
					...
					<option-notification
						class="w-full"
						:slack="slack"
						:option.sync="dados.funcao_alterada_para"
					></option-notification>
				</vx-card>
			</div>
			<div class="vx-col w-full lg:w-1/3 sm:w-1/2 mb-base">
				<vx-card title="Brinde" no-shadow>
					...
					<option-notification
						class="w-full"
						:slack="slack"
						:option.sync="dados.solicitacao_brinde_foi_aprovada"
					></option-notification>
				</vx-card>
			</div>
			<div class="vx-col w-full lg:w-1/3 sm:w-1/2 mb-base">
			...
			<div class="vx-col w-full lg:w-1/3 sm:w-1/2 mb-base">
				<vx-card title="Comissão" no-shadow>
					...
					<option-notification
						class="w-full"
						:slack="slack"
						:option.sync="dados.comissao_aprovada"
					></option-notification>
				</vx-card>
			</div>
			<div class="vx-col w-full lg:w-1/3 sm:w-1/2 mb-base">
				<vx-card title="Conquista" no-shadow>
					...
					<option-notification
						class="w-full"
						:slack="slack"
						:option.sync="dados.conquista_alcancada"
					></option-notification>
				</vx-card>
			</div>
			<div class="vx-col w-full lg:w-1/3 sm:w-1/2 mb-base">
				<vx-card title="Correios" no-shadow>
					...
					>Contrato do correios inativo (admins ou relacionados)</span
					>
					<option-notification
						class="w-full"
						:slack="slack"
						:option.sync="dados.contrato_correios_inativo"
					></option-notification>
				</vx-card>
			</div>
			...
			<div class="vx-col w-full lg:w-1/3 sm:w-1/2 mb-base">
				<vx-card title="Ticket" no-shadow>
					...
					<option-notification
						class="w-full"
						:slack="slack"
						:option.sync="dados.ticket_atrasados"
					></option-notification>
					<span class="font-bold"
					>Você tem um ticket agendado para logo</span
					>
					<option-notification
						:slack="slack"
						:option.sync="dados.ticket_agendado_para"
					></option-notification>
					<span class="font-bold">Seu ticket foi atendido</span>
					<option-notification
						:slack="slack"
						:option.sync="dados.ticket_foi_atendido"
					></option-notification>
				</vx-card>
			</div>
			<div class="vx-col w-full lg:w-1/3 sm:w-1/2 mb-base">
		...
		<transition name="fade">
			<footer-doug>
				<div class="vx-col sm:w-11/12 mb-2">
					...
					>
					<vs-button class="float-right mt-2" @click="update"
					>Salvar alterações
					</vs-button
					>
				</div>
...
export default {
	...
	methods: {
		...
		update() {
			this.$vs.loading();
			this.$store
				.dispatch("notificacoes/update", this.dados)
				.then(() => {
					this.$vs.loading.close();
				});
		},
		...
		verificaExtensao() {
			this.slack = this.$store.getters['extensoes/check']('slack');
		},
	},
};
</script>
