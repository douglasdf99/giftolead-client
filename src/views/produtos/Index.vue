				...
				<div class="flex items-center">
					<div class="relative w-full">
						...
						<form @submit="pesquisar">
							<vs-input autocomplete class="w-full vs-input-shadow-drop vs-input-no-border d-theme-input-dark-bg" v-model="dados.search" id="search_input" size="large" placeholder="Pesquisar por nome ou conta"/>
							<!-- SEARCH LOADING -->
							<!-- SEARCH ICON -->
							...
							<vs-tr :key="indextr" v-for="(tr, indextr) in data" class="mb-3">
								<vs-td class="flex justify-center items-center relative">
									<vs-dropdown vs-trigger-click v-if="checkPerm">
										<vs-button radius color="#EDEDED" type="filled" class="btn-more-icon relative botao-menu" icon-pack="material-icons" icon="more_horiz"></vs-button>
										<vs-dropdown-menu class="dropdown-menu-list">
											...
											<vs-dropdown-item @click="updateData(tr.id)"
																	v-if="$acl.check('configuracao_produto_editar')">
												<vs-icon icon-pack="material-icons" icon="create"></vs-icon>
												Editar
											</vs-dropdown-item>

											<vs-dropdown-item @click="deletar(data[indextr].id)"
																	v-if="$acl.check('configuracao_produto_deletar')">
								...
								<vs-td :data="data[indextr].cor">
									<div class="w-10 cursor-pointer h-10 rounded-lg m-2 float-left" :style="{backgroundColor: data[indextr].cor}" style="cursor: default !important;" />
								</vs-td>
...
export default {
	...
	methods: {
		...
		showLink(id) {
			this.$router.push({name: 'links-produto',params:{id}});
		},
		updateData(id) {
			this.$router.push({name: 'produto-editar', params:{id}});
		},
