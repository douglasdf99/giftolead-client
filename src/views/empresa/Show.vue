			...
			<div class="vx-col sm:w-2/3 w-full mb-2">
				...
				<div class="vx-row mb-6">
					<div class="vx-col w-full lg:w-1/2 mb-2">
						<span class="font-regular mb-2">Nome Fatansia</span>
						<input class="vs-inputx vs-input--input large hasValue" size="large" v-validate="'required'"
								 name="razao_social" v-model="empresa.nome_fantasia" type="text"/>
						<span class="text-danger text-sm"
								v-show="errors.has('nome_fantasia')">{{ errors.first('nome_fantasia') }}</span>
					</div>
					<div class="vx-col w-full lg:w-1/2 mb-2">
						<span class="font-regular mb-2">Apelido</span>
						<input class="vs-inputx vs-input--input large hasValue" size="large" v-validate="'required'"
								 name="razao_social" v-model="empresa.nome" type="text"/>
						<span class="text-danger text-sm"
				...
				<div class="vx-row mb-6">
					...
					<div class="vx-col sm:w-1/2 w-full mb-2">
						...
						<vs-input class="w-full" v-validate="'required|email'" name="email" v-model="empresa.email"
									 size="large"/>
						...
						<vx-card class="grid-view-item mb-base overflow-hidden" v-if="empresa.logo && !images.length">
							<template slot="no-body">
								...
								<div
									...
									<img :src="get_img_cdn(empresa.logo)" style="width: 200px" alt="logotipo"
				...
				<div class="vx-row mb-6">
					<div class="vx-col w-full mb-2">
						...
						<div>
							...
							<span class="text-danger text-sm" v-show="errors.has('descricao')">{{
				...
				<div class="vx-col sm:w-11/12 mb-2">
					<div class="container">
						<div class="vx-row mb-2 relative">
							<vs-button class="mr-3" color="primary" type="filled" @click="updateEmpresa"
										  v-if="$acl.check('configuracao_empresa_editar')">Salvar
							</vs-button>
							<vs-button class="mr-3" color="dark" type="flat" icon-pack="feather" icon="x-circle"
										  @click="$router.push({name: 'configuracoes-geral'})">Cancelar
							</vs-button>
						</div>
					</div>
				</div>
...
const dict = {
	custom: {
		...
		nome: {
			required: 'Por favor, insira um apelido',
		},
		nome_fantasia: {
...
export default {
	...
	methods: {
		...
		getCompany() {
			this.$vs.loading();
			this.$store.dispatch('empresa/getMyCompany')
				.then((response) => {
				...
				.catch(error => {
					...
					if (error.response.status == 404) this.$router.push({name: 'page-error-404'});
				});
		},
		...
		updateEmpresa() {
			this.$validator.validateAll().then(result => {
				if (result) {
					...
					formData.append('nome_fantasia', this.empresa.nome_fantasia);
					...
					this.$store.dispatch('empresa/updateCompany', formData)
						.then(() => {
							this.$vs.loading.close();
